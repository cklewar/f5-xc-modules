...
python3 -m pip -qqq --exists-action ignore --no-input --no-color install --progress-bar off --upgrade pip
python3 -m pip -qqq --exists-action ignore --no-input --no-color install --progress-bar off -r "$PWD/$MODULES_PATH/scripts/requirements.txt"
