...
resource "null_resource" "apply_site_update" {
  ...
  provisioner "local-exec" {
    command     = format("curl -v -X 'PUT' '%s/%s' -H 'Content-Type: application/json' -H 'Authorization: APIToken %s' -d '%s'", var.f5xc_api_url, local.site_update_uri, local.f5xc_api_token, local.content)
