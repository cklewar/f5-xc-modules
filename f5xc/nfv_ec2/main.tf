resource "aws_key_pair" "aws-key" {
  ...
  public_key = file(format("%s/keys/%s", path.module, var.PUBLIC_KEY_PATH))
}
...
resource "local_file" "payload" {
  ...
  filename = format("%s/_out/%s", path.module, var.aws_ec2_instance_userdata_file)
}
...
resource "aws_instance" "instance" {
  ...
  connection {
    ...
    user        = "ubuntu"
    private_key = file(format("%s/keys/%s", path.module, var.PRIVATE_KEY_PATH))
  }
