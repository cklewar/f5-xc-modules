...
python3 "$MODULES_ROOT"/$MODULES_PATH/scripts/script.py delete $api_url $api_token $tenant $api_credentials_name "$MODULES_ROOT" "s3" -r "$aws_region" -b "$s3_bucket" -k "$s3_key"
