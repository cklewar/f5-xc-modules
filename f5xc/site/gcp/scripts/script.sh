...
INSTANCE_NAME=$(./scripts/hcl2json test.hcl | jq '.instance_names[0]')
MASTER_PRIVATE_IP_ADDRESS=$(./scripts/hcl2json test.hcl | jq '.master_private_ip_address.'"$INSTANCE_NAME")
MASTER_PUBLIC_IP_ADDRESS=$(./scripts/hcl2json test.hcl | jq '.master_public_ip_address.'"$INSTANCE_NAME")
