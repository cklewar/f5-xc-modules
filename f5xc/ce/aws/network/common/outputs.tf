output "common" {
  value = {
    ...
    sg_sli           = var.f5xc_ce_gateway_type == var.f5xc_ce_gateway_type_ingress_egress ? module.aws_security_group_sli.aws_security_group : null
    existing_vpc     = var.aws_existing_vpc_id != "" ? data.aws_vpc.vpc : null
    nat_gw_eip_id    = var.f5xc_ce_gateway_type == var.f5xc_ce_gateway_type_ingress_egress ? aws_eip.nat_gw_eip[0].id : null
