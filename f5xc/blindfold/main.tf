resource "null_resource" "action_secret_policy_rule" {
  ...
  provisioner "local-exec" {
    command     = format("curl -v -X 'POST' '%s/%s' -H 'Content-Type: application/json' -H 'Authorization: APIToken %s' -d '%s'", var.f5xc_api_url, local.f5xc_secret_management_secret_policy_rules_post_uri, local.f5xc_api_token, local.f5xc_blindfold_policy_rule_payload)
...
resource "null_resource" "action_secret_policys" {
  ...
  provisioner "local-exec" {
    command     = format("curl -v -X 'POST' '%s/%s' -H 'Content-Type: application/json' -H 'Authorization: APIToken %s' -d '%s'", var.f5xc_api_url, local.f5xc_secret_management_secret_policy_post_uri, local.f5xc_api_token, local.f5xc_blindfold_policys_payload)
    interpreter = ["/usr/bin/env", "bash", "-c"]
  }
