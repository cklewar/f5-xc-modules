locals {
  ...
  nfv_virtual_server_ip = var.f5xc_nfv_type == var.f5xc_nfv_type_f5_big_ip_aws_service ? (jsondecode(data.http.nfv_virtual_server_ip.*.response_body).status[*].vip)[0] : null
}