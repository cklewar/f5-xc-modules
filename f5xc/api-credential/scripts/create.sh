...
then
  python3 "$MODULES_ROOT"/$MODULES_PATH/scripts/script.py post "$api_url" "$api_token" "$tenant" "$api_credentials_name" "$MODULES_ROOT" -c "$api_credential_type" -v "$virtual_k8s_name" -n $virtual_k8s_namespace -e $api_credential_expiry_days
else
  python3 "$MODULES_ROOT"/$MODULES_PATH/scripts/script.py post "$api_url" "$api_token" "$tenant" "$api_credentials_name" "$MODULES_ROOT" -c "$api_credential_type" -p "$api_credential_password" -e $api_credential_expiry_days
fi
