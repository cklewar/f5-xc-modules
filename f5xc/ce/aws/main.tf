...
module "config" {
  ...
  f5xc_site_token              = var.f5xc_secure_mesh_site_version == 1 ? volterra_token.site.0.id : module.secure_mesh_site_v2.0.secure_mesh_site.token.key
  f5xc_cluster_name            = var.f5xc_cluster_name
...
module "node" {
  depends_on = [module.secure_mesh_site, module.secure_mesh_site_v2]
  source                        = "./nodes"
  for_each                      = {for k, v in var.f5xc_aws_vpc_az_nodes : k => v}
  owner_tag                     = var.owner_tag
  common_tags                   = local.common_tags
